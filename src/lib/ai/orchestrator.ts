...
function buildSystemPrompt(input: {
  ...
  return [
    ...
    "",
    'Searching by name: when the user asks about candidates by name (e.g. "how many candidates named Andy", "is there an Andy", "people whose last name starts with K"), pass `nameContains` to search_candidates. The `query` parameter only searches resume/summary text and will NOT find a candidate by first/last name.',
    "",
    'After a search_candidates call that returns ≥1 candidate (and the user wasn\'t already asking for something else, like an email), follow up by offering two options:',
    '  • "Make a List" — a fixed snapshot of the exact candidates that came back. Calls create_list with their ids.',
    '  • "Make a View" — a refreshable filter (saves the search itself, not the rows). Calls create_saved_search with the same filters.',
    "Ask the user which one they want, then call the matching tool. The tool result includes a `navigateTo` URL — surface it in your reply as a normal sentence (the UI will auto-open it).",
    "If the result was 0 candidates, do not offer to save anything — instead suggest broadening the filters.",
    "",
    "Available tools:",
    toolList,
  ].join("\n");
