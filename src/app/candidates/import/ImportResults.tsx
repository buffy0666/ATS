...
import { type ImportResult, type RowResult } from "./import-types";

/** Shared results panel for both the template and field-mapping importers. */
    ...
    <section className="mt-6">
      ...
      <div className="overflow-hidden rounded-lg border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        <table className="w-full text-sm">
          ...
          <tbody>
            {result.rows.map((r) => (
              <tr key={`${r.row}-${r.status}`} className="border-t border-zinc-200 dark:border-zinc-800">
...
function Stat({ label, value, tone }: { label: string; value: number; tone: "ok" | "muted" | "err" }) {
  const toneClass =
    tone === "ok"
...
export function downloadErroredRows(result: ImportResult) {
