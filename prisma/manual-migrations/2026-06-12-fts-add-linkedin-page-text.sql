-- Candidate FTS trigger: add linkedinPageText to the searchVector index
-- (weight C, alongside notes/resumeText). Candidates captured via the
-- Chrome extension often have ALL their useful text (company, title,
-- experience) only in linkedinPageText until AI processing fills the
-- structured fields — without this they're invisible to search.
--
-- Applied to production Supabase on 2026-06-12 via
-- scripts/apply-manual-migration.ts. Kept in the repo so future
-- re-creates of the DB can see the current state of the trigger.
--
...
BEGIN
  NEW."searchVector" :=
    ...
    setweight(to_tsvector('english', coalesce(NEW."linkedinPageText",'')), 'C');
  RETURN NEW;
END
$$;
...
BEFORE INSERT OR UPDATE OF
  ...
  notes, "resumeText", "linkedinPageText"
ON public."Candidate"
FOR EACH ROW EXECUTE FUNCTION candidate_search_vector_update();

-- Backfill: re-vector only the rows that have LinkedIn page text — the
-- others are unaffected by this change.
UPDATE "Candidate" SET "firstName" = "firstName" WHERE "linkedinPageText" IS NOT NULL;
